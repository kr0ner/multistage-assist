...
class EntityResolverCapability(Capability):
    ...
    def _find_area(self, area_name: Optional[str]):
        ...
        for a in area_reg.async_list_areas():
            if self._canon(a.name or "") == needle: return a
        return None
    ...
    def _entities_in_area(self, area, domain: Optional[str]) -> List[str]:
        ...
        for ent in ent_reg.entities.values():
            ...
            if not in_area and not has_any_area:
                ...
                if canon_area and (canon_area in name_match or canon_area in eid_match): in_area = True
            if not in_area: continue
            if domain and ent.domain != domain: continue
    ...
    def _collect_by_name_exact(self, hass, name, domain, all_entities) -> List[str]:
        ...
        for eid, ent in all_entities.items():
            if domain and not eid.startswith(f"{domain}."): continue
    ...
    def _collect_by_name_fuzzy(self, hass, name, domain, fuzz_mod, all_entities, allowed=None) -> List[str]:
        needle = self._canon(name)
        if not needle: return []
        scored: List[Tuple[str, int, str]] = []
        for eid, ent in all_entities.items():
            if not eid.startswith(f"{domain}."): continue
            if allowed is not None and eid not in allowed: continue
            st = hass.states.get(eid)
            friendly = st and st.attributes.get("friendly_name")
