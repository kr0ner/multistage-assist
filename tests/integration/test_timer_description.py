...
async def test_timer_description_extraction():
    ...
    for input_text, expected_desc in test_cases:
        ...
        # For empty expectation, accept empty or LLM noise (duration/timer echoes)
        else:
            noise_words = ["minuten", "sekunden", "stunden", "timer", "wecker", "countdown"]
            is_noise = any(w in desc.lower() for w in noise_words)
            assert len(desc) < 5 or is_noise, \
                f"Expected empty/noise description, got '{desc}'"
