...
# Import from utils
from custom_components.multistage_assist.conversation_utils import format_chat_history
...
""",
    ...
    async def _get_client(self) -> GoogleGeminiClient | None:
        async with self._init_lock:
            if self._client_wrapper: return self._client_wrapper
    ...
    async def run(self, user_input, history: List[Dict[str, str]] = None, **_: Any) -> conversation.ConversationResult:
        ...
        if not client:
            ...
            # Use utility
            context_str = format_chat_history(history, max_words=500) if history else f"User: {current_text}"
