...
from homeassistant.const import (
    UnitOfTemperature, UnitOfPower, UnitOfEnergy, PERCENTAGE
)
...
LIGHT_KEYWORDS: Dict[str, str] = {
    ...
    "beleuchtung": "beleuchtungen", "spot": "spots",
}
COVER_KEYWORDS: Dict[str, str] = {
    "rollladen": "rollläden", "rollo": "rollos", "jalousie": "jalousien",
    "markise": "markisen", "beschattung": "beschattungen",
}
SWITCH_KEYWORDS: Dict[str, str] = {
    "steckdose": "steckdosen", "schalter": "schalter",
    "zwischenstecker": "zwischenstecker", "strom": "strom",
}
FAN_KEYWORDS: Dict[str, str] = {
    "ventilator": "ventilatoren", "luefter": "luefter", "lüfter": "lüfter",
}
MEDIA_KEYWORDS: Dict[str, str] = {
    "tv": "tvs", "fernseher": "fernseher", "musik": "musik",
    "radio": "radios", "lautsprecher": "lautsprecher", "player": "player",
}
SENSOR_KEYWORDS: Dict[str, str] = {
    "temperatur": "temperaturen", "luftfeuchtigkeit": "luftfeuchtigkeiten",
    "feuchtigkeit": "feuchtigkeiten", "wert": "werte",
    "status": "status", "zustand": "zustände",
}
...
OTHER_ENTITY_PLURALS: Dict[str, str] = {
    "das fenster": "die fenster", "tür": "türen", "tor": "tore", "gerät": "geräte",
}
...
_PLURAL_CUES = {"alle", "sämtliche", "mehrere", "beide", "beiden", "viele", "verschiedene"}
_NUM_WORDS = {"zwei", "drei", "vier", "fünf", "sechs", "sieben", "acht", "neun", "zehn", "elf", "zwölf"}
...
async def make_response(message: str, user_input: conversation.ConversationInput, end: bool = False):
    ...
    return conversation.ConversationResult(
        response=resp, conversation_id=user_input.conversation_id, continue_conversation=not end
    )

async def error_response(user_input: conversation.ConversationInput, msg: str = None):
    return await make_response(msg or "Entschuldigung, ich habe das nicht verstanden.", user_input)
...
def with_new_text(user_input: conversation.ConversationInput, new_text: str) -> conversation.ConversationInput:
    ...
    return conversation.ConversationInput(
        text=new_text, context=user_input.context, conversation_id=user_input.conversation_id,
        device_id=user_input.device_id, language=user_input.language,
        agent_id=getattr(user_input, "agent_id", None), satellite_id=satellite_id,
    )
...
def normalize_speech_for_tts(text: str) -> str:
    ...
    replacements = {
        "°C": " Grad Celsius", "°": " Grad", "%": " Prozent",
        "kWh": " Kilowattstunden", "kW": " Kilowatt", "W": " Watt",
        "V": " Volt", "A": " Ampere", "lx": " Lux", "lm": " Lumen",
    }
...
def parse_duration_string(duration: Any) -> int:
    ...
    if isinstance(duration, str):
        ...
        total = 0
        if m_hr: total += int(m_hr.group(1)) * 3600
        if m_min: total += int(m_min.group(1)) * 60
        if m_sec: total += int(m_sec.group(1))
        if total == 0 and text.isdigit(): return int(text) * 60
        return total
    return 0

def format_seconds_to_string(seconds: int) -> str:
    if seconds >= 3600: return f"{seconds/3600:.1f} Stunden"
    if seconds >= 60: return f"{int(seconds/60)} Minuten"
